...
def test_rigid_component_transform_rotates_paths_text_and_dimensions() -> None:
    ...
    assert rotated.paths[0].points[0].x == pytest.approx(0)
    assert rotated.paths[0].points[0].y == pytest.approx(0)
    assert rotated.text_frames[0].x == pytest.approx(-20.4)
    assert rotated.text_frames[0].y == pytest.approx(10)
    assert rotated.text_frames[0].rotation == pytest.approx(95)
    assert rotated.item_order == component.item_order


def test_affine_transform_rotates_bezier_handles_with_their_anchor() -> None:
    ...
    rotated = AffineTransform.rotation(90)
    transformed = RenderedComponent(width=20, height=10, paths=[ellipse]).transformed(rotated)
...
def test_component_with_text_rejects_non_rigid_scale() -> None:
